...
def generalGroupQuestionnaire(request,id_group,id_cue):
	...
	script = "google.charts.load('current', {'packages': ['corechart']});google.charts.setOnLoadCallback(drawChart);function drawChart(){var data = google.visualization.arrayToDataTable(["
	script += "['Campo', 'Calificaciones'],"
	script += "['Reprobados', {}],".format(valueTopic[1])
	valueInfo += [['Campo', 'Calificaciones'], ['Reprobados', valueTopic[1]], ['Aprobados', valueTopic[0]]]
	...
	return JsonResponse(valueInfo, safe=False)
...
def generalColumnGroupTopic(request,id_group):
	...
	valueInfo += [['Tema', 'Correctos', 'Incorrectos']]
	for keys in temas:
		script += "['{}',{},{}],".format(dbprocess.getNameTopic(keys),str(temas[keys][0]),str(temas[keys][1]))
		valueInfo += [[dbprocess.getNameTopic(keys),str(temas[keys][0]),str(temas[keys][1])]]
	script += "]);"
...
def generalColumnTopic(request):
	...
	for keys in temas:
		script += "['{}',{},{}],".format(dbprocess.getNameTopic(keys),str(temas[keys][0]),str(temas[keys][1]))
		valueInfo += [[dbprocess.getNameTopic(keys),temas[keys][0],temas[keys][1]]]
	script += "]);"
	script += "var options = { chart: {"
	script += "title: 'Temas'"
	script += ",subtitle: 'Aciertos mas Correctos y Incorrectos en Temas',}, colors: ['#ff9800','#5c6bc0']};"
...
def questionnaireGradeGroup(request,id_group,id_cue):
	...
	valueInfo = [['Preguntas', 'Correctos', 'Incorrectos']]
	for keys in temas:
		script += "['{}',{},{}],".format(dbprocess.getNameQuestion(keys),str(temas[keys][0]),str(temas[keys][1]))
		valueInfo += [[dbprocess.getNameQuestion(keys), temas[keys][0], temas[keys][1]]]
	script += "]);"
	script += "var options = { chart: {"
...
def questionnaireAlumno(request,id_alumno,id_cue):
	...
	valueInfo = [['Campo', 'Calificación cuestionario'], ['Correctas', len(answers[0])], ['Incorrectas', len(answers[1])]]
	script += "]);var options = {"
	script += "title: '{}',is3D: true,".format(nameCuest)
...
def questionnairesAlumnoTopic(request,id_alumno,id_tem):
	...
	valueInfo = [['Campo', 'Calificación cuestionario'], ['Correctas', len(correctas)], ['Incorrectas', len(incorrectas)]]
	script += "]);var options = {"
